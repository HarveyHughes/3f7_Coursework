...
def encode(x, p):
    ...
    for k in range(len(x)): # for every symbol
        ...
        while True:
            if hi < half: # if lo < hi < 1/2
                ...
                for a in range(straddle): y.append(1) # extend by a sequence of 'straddle' ones
                ...
                for a in range(straddle): y.append(0) # extend 'straddle' zeros
    ...
    if lo < quarter: # the position of lo determines the dyadic interval that fits
        ...
        for a in range(straddle): y.append(1)
    else:
        y.append(1) # output a 1 followed by "straddle" zeros
        for a in range(straddle): y.append(0)
    return(y)
